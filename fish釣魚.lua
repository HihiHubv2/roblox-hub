...
local config = {
    ...
    shakeSpeed = 0.05,
    FreezeWhileFishing = false,  -- 不再需要控制凍結
    isFishingActive = true -- 新增這個變數來控制自動釣魚功能是否啟用
}
...
-- 關閉按鈕，停止自動釣魚
local CloseButton = Tabs.Main:AddButton({
    Title = "Stop Fishing",
    Description = "Click to stop the fishing automation",
    Callback = function()
        config.isFishingActive = false  -- 停止自動釣魚
        print("Fishing automation stopped.")
    end
})

-- 主循環
while task.wait(config.shakeSpeed) do
    if config.isFishingActive then  -- 檢查自動釣魚是否啟動
        local rod = farm.find_rod()
        if rod then
            -- 不再凍結角色，讓角色始終可動
            farm.cast()
