...
Builder.new():root("HihiHub", "Notoriety"):library(library):withSaveManager(savemanager):withThemeManager(thememanager):windows({ Window.new():title("HihiHub|v.0.1"):centered(true):autoShow(true):withFadeTime(0):pages({ Page.new():title("Gameplay"):left({ Groupbox.new():title("NPC Utility"):elements({ Toggle.new("gameplay.kill_cops.enabled"):title("Kill All Cops"):tooltip("Automatically kills cops"):default(false), DependencyBox.new():dependsOn("gameplay.kill_cops.enabled", true):elements({ Slider.new("gameplay.kill_cops.delay"):title("Kill All Cops Delay"):suffix(" seconds"):round(1):limits(0, 1):default(0.1):hideMax(true) }), Toggle.new("gameplay.yell_civilians.enabled"):title("Yell at all civillians"):tooltip("Automatically yells at civillians"):default(false), DependencyBox.new():dependsOn("gameplay.yell_civilians.enabled", true):elements({ Slider.new("gameplay.yell_civilians.delay"):title("Yell at all civillians Delay"):suffix(" seconds"):round(1):limits(0, 1):default(0.1):hideMax(true) }) }), Groupbox.new():title("Players Utility"):elements({ Toggle.new("gameplay.movement.infinite_stamina"):title("Infinite Stamina"):tooltip("Disables stamina consumption"):default(false), Button.new("gameplay.player.self_revive"):title("Self-Revive"):tooltip("Revives yourself in an instant"):onClick(function()
